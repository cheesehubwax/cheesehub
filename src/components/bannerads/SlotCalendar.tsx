                    ...
                    return (
                      <div key={slot.position} className={`flex flex-col sm:flex-row sm:items-center justify-between gap-2 rounded-lg border p-3 bg-background/50 transition-colors ${selected ? "border-cheese/60 bg-cheese/5" : "border-border/30"}`}>
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                          ...
                          {slot.isOnChain && !slot.suspended && ({slot.isOnChain && !slot.suspended && (slot.user === accountName || slot.sharedUser === accountName) && isWithinBuffer(slot.time, MIN_RENT_BUFFER_HOURS) && <Button size="sm" variant="outline" className="border-cheese/30 text-cheese text-xs h-7" onClick={() => setEditTarget(slot)}>Edit</Button>}) && <Button size="sm" variant="outline" className="border-cheese/30 text-cheese text-xs h-7" onClick={() => setEditTarget(slot)}>Edit</Button>}
