...
1. **Self-powerup** — `cheeselardr1` -> `cheesepowerz`, 1 CHEESE, memo `cheeselardr1`,
...
5. Sends `1.0000 CHEESE` from `cheeselardr1` -> `cheesepowerz` with the staker's
...
| Name                    | Where                       | Purpose                                       |
| ----------------------- | --------------------------- | --------------------------------------------- |
| `WAX_SIGNER_ACCOUNT`    | GH repo variable (`vars`)   | Signing WAX account. Default: `cheeselardr1`. |
...
1. Create a new permission `dailypower` on `cheeselardr1`, parent `active`,
...
4. Add repo variables `WAX_SIGNER_ACCOUNT=cheeselardr1` and
   `WAX_SIGNER_PERMISSION=dailypower`.
5. Keep `cheeselardr1` topped up with a small baseline of staked WAX so the
