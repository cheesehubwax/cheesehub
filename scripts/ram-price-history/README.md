...
Records the WAX RAM price (and its CHEESE / USD equivalents) every 4 hours so
...
- Data lives on the dedicated `ram-price-data` branch at
  `data/ram-price-history.json`. Keeping it off `main` means the 4-hourly commits
...
capped at 4400 records (~2 years).
...
Set `FORCE=1` to bypass the "last sample is under 2 hours old" guard.
...
`.github/workflows/ram-price-history.yml` runs on `cron: "7 */4 * * *"` and can
be triggered manually via **Run workflow**. It creates the `ram-price-data`
branch on first run, appends the sample, and commits only when the file changed.
